...
\begin{itemize}
    ...
    \item In parallel, some entity adjusts weights connecting the node $\fnmroot$ thus setting strategic objectives.
\end{itemize}
...
be some entity that does so, we do not elaborate on details of its implementation. Neither do we reveal how exactly
agents' tactical reasoning processes are implemented, and what are the rules of decomposing an ultimate goal that the
swarm pursuits. Those are extension points to a particular implementation, and we leave it beyond consideration.
